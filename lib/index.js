...
class AwsIPAddressLookup {
  ...
  lookup(addresses, callback) {
    ...
    request.get(this.awsQueryURL, options, (error, response, body) => {
      ...
      addrs.forEach((addr) => {
        ...
        if (!err && !address4.isValid() && !address6.isValid()) {
          ...
          let details = {
            ipAddress: addr,
            ipPrefix: ipRange && (ipRange.ip_prefix || ipRange.ipv6_prefix) || '',
    ...
    return callback(null, typeof addresses === 'string' && addrDetails[0] || addrDetails);
    });
  }
}

module.exports = AwsIPAddressLookup;
